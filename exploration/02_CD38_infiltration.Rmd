...
CD38_plot_TI_total <- ggboxplot(PPBC_metadata, x="group", y="TI_CD38_total",
          ...
          order = c("PPBCPW", "PPBCDL", "PrBC", "NPBC"),
          ylab = "CD38 score", xlab = "Study Group") +
  geom_dotplot(binaxis = "y", stackdir = "center", position = "dodge", binwidth = 1, aes(colour = group)) + stat_compare_means(method = "wilcox.test", comparisons = my_comparisons) + stat_compare_means(label.y = 60) + ylim(0,60) # + stat_pvalue_manual(pwc_delta_estimate_CD3CD8pos_CD20, hide.ns = TRUE, y.position = c(1.25))
CD38_plot_TI_total
```
