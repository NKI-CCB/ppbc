...
```{r}
clust_summary <- full_join(select(clust, sample_name, clust.new = IG_col_cluster, study_group.new = study_group),
          select(old.clust, sample_name, clust.old = IG_col_cluster, study_group.old = study_group),
          by = "sample_name")
...
```{r}
table(clust_summary$study_group.new, clust_summary$clust.new) %>%
  addmargins()
```

```{r}
table(clust_summary$study_group.new, clust_summary$clust.old) %>%
  addmargins()
```
...
sd.comp <- full_join(
  sd %>% select(sample_name, batch.new = batch, timeOS.new=time_OS_months, death.new=death,
                DR.new = distant_recurrence, timeDR.new=time_DRS_months, study_group.new=study_group,
                PAM50.new=PAM50),
  sd.old %>% select(sample_name, batch.old=batch,timeOS.old = months_overall_survival,
                  ...
                  study_group.old=study_group, PAM50.old=PAM50),
  by="sample_name")
```
...
```{r}
length(unique(sd$sample_name)) == length(unique(sd.old$sample_name))
all(sort(sd$sample_name) == sort(sd.old$sample_name))
```


```{r}
sd.comp %>%
  dplyr::filter(PAM50.new != PAM50.old)
```
