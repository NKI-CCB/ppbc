...
mult_cox <- lapply(relevant_cells, function(x){
  ...
  # group_by(panel) %>% #Debatable
  mutate(lrt.fdr = p.adjust(lrt.pval, method = "BH"), .after = panel) %>%
  mutate(formula = "Surv(time = FU_time_months, event = death) ~ strata(group) + stage + density")
...
inv_mult_cox <- lapply(relevant_cells, function(x){
  all_cox_density(inv_dens, x,
                  f = "Surv(time = FU_time_months, event = death) ~  stage")
}) %>%
  bind_rows() %>%
  # group_by(panel) %>% #Debatable
  mutate(lrt.fdr = p.adjust(lrt.pval, method = "BH"), .after = panel) %>%
  mutate(formula = "Surv(time = FU_time_months, event = death) ~  stage + density")
...
interaction_df <- lapply(relevant_cells, function(x){
 ...
 bind_rows() %>%
  ...
  mutate(lrt.fdr = p.adjust(lrt.pval, method = "BH"), .after = panel) %>%
  mutate(formula = "Surv(time = FU_time_months, event = death) ~ stage + group:density")
