...
List all existing combinations of markers, their location (tumor vs stroma), by sample and by panel.

```{r load-counts}
cell_counts <- read_csv(here("results/spatial/cell_counts_by_marker.csv"))
cell_counts %>%
  group_by(marker_pos, panel, classifier_label) %>%
  summarise(max_n=max(n), .groups="drop") %>%
  left_join(cell_counts, by=c("marker_pos", "panel", "classifier_label", max_n="n")) %>%
  group_by(marker_pos, panel, classifier_label, max_n) %>%
  summarise(t_number=paste0(t_number, collapse="/"), .groups='drop') %>%
  pivot_wider(
    names_from=classifier_label,
    values_from=c(max_n, t_number)) %>%
  arrange(desc(max_n_Stroma + max_n_Tumor))
```
...
mpif26 <- dir_ls(here("data/vectra/interim/objects/"), regexp = ".*MPIF26.*\\.nc") %>%
  map_dfr(~ read_cells(., intensity=T), .id="file") %>%
