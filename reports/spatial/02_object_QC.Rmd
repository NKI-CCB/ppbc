...
```{r}
mpif26 %>%
  #discard intensities, keeping logicals and spatial data
  select(-CD20_nuclear:-PanCK_membrane) %>%
  saveRDS(., here("data/vectra/interim/mpif26_df.Rds"))
```

```{r}
mpif27 %>%
  #discard intensities, keeping logicals and spatial data
  select(-CD138_nuclear:-PanCK_membrane) %>%
  saveRDS(., here("data/vectra/interim/mpif27_df.Rds"))
```
