...
```{r}
dir.create(here("data/vectra/symlinks"), showWarnings = F)
dir.create(here("data/vectra/symlinks/annotations"), showWarnings = F)
dir.create(here("data/vectra/symlinks/objects"), showWarnings = F)
dir.create(here("data/vectra/symlinks/summary"), showWarnings = F)
```

We'll be using the markers instead of the panel ID in the file name. This should allow selecting the panel via pattern searches.

```{r}
MPIF26 <- "CD3-CD20-CD27-FoxP3-PanCK"
MPIF27 <- "CD3-CD8-CD138-CD20-PanCK"
```
...
```{r}
symdir <- here("data/vectra/symlinks")
...
filedict$sym_mpif27_object_results <- file.path(
  symdir, "objects", paste(filedict$t_number, MPIF27, "object_results.csv", sep = "_")
  )
```
...
filedict$sym_mpif26_summary_results <- file.path(
  symdir, "summary", paste(filedict$t_number, MPIF26, "summary_results.csv", sep = "_")
  )

filedict$sym_mpif27_summary_results <- file.path(
  symdir, "summary", paste(filedict$t_number, MPIF27, "summary_results.csv", sep = "_")
  )
```
...
filedict$sym_mpif26_annotations <- file.path(
  symdir, "annotations", paste(filedict$t_number, MPIF26, "annotations.xml", sep = "_")
  )

filedict$sym_mpif27_annotations <- file.path(
  symdir, "annotations", paste(filedict$t_number, MPIF27, "annotations.xml", sep = "_")
  )
```
...
file.path(
  symdir, "annotations", "job_annotations", paste(filedict$t_number, MPIF26, "job_annotations.xml", sep = "_")
  )

filedict$sym_mpif27_job_annotations <- 
file.path(
  symdir, "annotations", "job_annotations", paste(filedict$t_number, MPIF27, "job_annotations.xml", sep = "_")
  )
```
