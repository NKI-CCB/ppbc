...
Check that disallowed marker pairs indeed do not appear together.

```{r no-more-disallowed-pairs-mpif26}
print(disallowed_marker_pairs)
for(i in 1:length(disallowed_marker_pairs)){
  ...
  if (!(m1 %in% colnames(mpif26_corrected) & m2 %in% colnames(mpif26_corrected))) {
    next
  }
...
```{r no-more-dissallowed-pairs-mpif27}
for(i in 1:length(disallowed_marker_pairs)){
  m1 <- paste0(disallowed_marker_pairs[[i]][1], "_positive")
  m2 <- paste0(disallowed_marker_pairs[[i]][2], "_positive")
  if (!(m1 %in% colnames(mpif27_corrected) & m2 %in% colnames(mpif27_corrected))) {
    next
  }
