...
These models were run in script 15.

```{r}
resDir <- here("data", "Rds")

glm.os1000 <- readRDS(file.path(resDir,"15_glm_os_1000.Rds"))

glm.os5000 <- readRDS(file.path(resDir,"15_glm_os_5000.Rds"))

glm.drs1000 <- readRDS(file.path(resDir,"15_glm_drs_1000.Rds"))

glm.drs5000 <- readRDS(file.path(resDir,"15_glm_drs_5000.Rds"))
...
```{r, include=F}
pdf(here("results", "survival", "15_hist_features_cox_enet.pdf"), height=6, width=9)
...
```{r}
ggsave(here("results", "survival", "15_cox_predictions.pdf"), height=6, width = 9)
```
...
```{r}
filter(gx_annot, ensembl_gene_id == Reduce(intersect, overlap[names(overlap)!="uni_genewise_os"]))
filter(gx_annot, ensembl_gene_id == Reduce(intersect, overlap[names(overlap)!="multi_genewise_drs"]))
filter(gx_annot, ensembl_gene_id == Reduce(intersect, overlap[!names(overlap)%in%
                                                                c("uni_genewise_os", "drs1000", "os1000")]))
...
openxlsx::write.xlsx(lapply(glm.features, function(x){x[[1]]}),
                     file = here("results", "survival", "15_elastic_cox_features.xlsx"))

write_csv(spear_cor, here("results/survival/15_enetcox_pvals.csv"))
```
