...
```{r, collapse=T}
print(paste("Interaction model identifies", nrow(inv_int_os %>% filter(fdr < 0.05 & gene_type == "protein_coding")), "protein coding genes at fdr < 0.05"))

inv_int_os %>% filter(fdr < 0.05 & gene_type == "protein_coding") %>% pull(gene_name)
```
```{r, fig.height = 10, fig.width=8}
print(ggarrange(plotlist = km_ntiles("BCAR4", survival_type = "os"), ncol = 1, nrow = 2)) 
```
...
```{r, fig.height = 10, fig.width=8}
for (i in inv_int_os %>% filter(fdr < 0.05 & gene_type == "protein_coding") %>% head(20) %>% pull(gene_name)){
...
Using a different function we can see 1 vs rest comparisons.
...
for (i in (inv_int_os %>%
           filter(fdr < 0.05 & gene_type == "protein_coding") %>%
           head(20) %>% pull(gene_name))){
  print(km_ntiles_ovr(i, ovr_column = "involution", survival_type = "os",
                      sampledata =  mutate(sample_data, involution = if_else(study_group == "ppbc_inv", 1, 0))))
}

```
```{r,fig.height=8, fig.width=10}
km_ntiles_ovr("BCAR4", ovr_column = "involution", survival_type = "os",
                      sampledata =  mutate(sample_data, involution = if_else(study_group == "ppbc_inv", 1, 0)))
```
