...
pathlist = list()
#pathlist[[names(repLRT)[1]]] = 
pathlist = repLRT$pathways %>% bind_rows() %>% mutate(comp = "LRT")
...
```{r}
ggsave(filename=here("results","diffex", "figs", "06_LRT",
                     "LRT.outliers.jpeg"), plot=repLRT$volc_outliers,height = 6, width = 8)
...
for(i in 1:length(repLRT$pathway_plots)){
  thisplot = repLRT$pathway_plots[[i]]
  thisplotname = names(repLRT$pathway_plots)[i]
  
  file=paste0(paste("LRT", thisplotname, sep="-"),".jpeg")
  ggsave(filename=here("results","diffex", "figs",
                       "06_LRT",file),
           plot=thisplot, height = 6, width = 8)
  }
