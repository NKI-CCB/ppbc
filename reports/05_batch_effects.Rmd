...
```{r}
print(paste("Percent variance represented:", round(sum(percVar[percVar$PC %in% PC_batch$PC, "variance"]), 2)))
```
...
```{r}
print(paste("Percent variance represented:", round(sum(percVar[percVar$PC %in% PC_studygroup$PC, "variance"]), 2)))
```
...
It appears that the lac group may be over-contributing. Can we still find PCs associated with study group when lac is removed?
...
for (i in 1:nrow(corvars_cat)){
  ...
  tbl = table(sd[,thisx], sd[,thisy])
  chires =chisq.test(sd[,thisx], sd[,thisy], simulate.p.value = T)
  corrplot::corrplot(chires$residuals, is.corr=F, cl.cex = 1.5, cl.align.text = "l")
}
```
