...
```{r}
cor_plot <- function(df=figuredata, x, y, colorby, met, ht = 1, wd = 0.2, ...){
...
TIL_CD4_cor_all <- 
  ...
  cor_plot(figuredata, x= "CD4naive_fraction", y="TILpercent", colorby = "study_group",
           met = "spearman", ht = 0, wd = 0) +
    scale_color_manual(values = study_colors[names(study_colors) != "rest"]) +
    ggtitle("Spearman correlation between TAPCs and TILs in all samples")
