...
multivariate_km <- function(df, event_type = c("OS", "DRS"),
  ...
  if(stage_as_integer){
    df <- df %>%
      mutate(stage_factor = stage, .after= stage) %>%
  ...
  df <- df[,c("study_group", "sample_name", "stage", #"stage_factor",
              "grade", "clin_subtype",
...
supfig2c <- multivariate_km(figuredata, event_type = "OS",
                            stage_as_integer = F, returndata = F) +
...
```{r}
supfig2d <- multivariate_km(figuredata, event_type = "DRS",
                            stage_as_integer=F, returndata = F, returncox = F) +
