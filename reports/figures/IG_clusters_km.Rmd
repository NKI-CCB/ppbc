...
univariate_km <- function(df, event_type = c("OS", "DRS"), returndata = F){
  ...
  ggsurvplot(
    ...
    ggtheme = theme_bw(),
    pval = T,
    legend.labs = names(study_colors),
...
For p values I used the coefficient for PPBCpw in the Cox regression model with these covariates.
...
multivariate_km <- function(df, event_type = c("OS", "DRS"),
  ...
  df <- df[,c("study_group", "sample_name", "stage", "grade", "PAM50",
  ...
  if(event_type == "OS"){
    ...
    study_strata <- coxph(
      Surv(time=time_OS_months,
           event=death) ~ stage + PAM50 + strata(study_group), data = df
      )
    
    fitvsNPBC <- coxph(
      Surv(time=time_OS_months, event=death) ~ stage + PAM50 + study_group,
    ...
    fitvsNPBC <- coxph(
      Surv(time=time_DRS_months, event=distant_recurrence) ~ stage + PAM50 + study_group,
      data = as.data.frame(mutate(df, study_group = relevel(study_group, ref = "NP-BC")))
      )
    ...
    study_strata <- coxph(
      Surv(time=time_DRS_months, event=distant_recurrence) ~ stage + PAM50 + strata(study_group),
      data = df
      )
  }
