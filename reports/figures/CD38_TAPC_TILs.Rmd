...
```{r}
TAPC_CD38_cor_all <- figuredata %>%
  filter(!is.na(TAPC_score) & !is.na(TI_CD38_total)) %>%
  ggplot(aes(x = TAPC_score, y = TI_CD38_total)) +
  geom_jitter(aes(color = study_group), height = 1, width = 0.2, shape = 15) +
  ...
  geom_smooth(method = "lm", formula = y ~ x) +
      ggpubr::stat_cor(method = "kendall", #label.y = 3.5, label.x = -0.2,
                       aes(label = paste(..rr.label.., ..r.label.., ..p.label..,
                                         sep = "~`,`~"))) +
  ylab("% CD38 positive cells") +
  xlab("TAPC score") + labs(color = "Study group") +
