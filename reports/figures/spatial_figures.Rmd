...
```{r}
rt_fig2c <- faceted_risktable(fig2c, faceted.by = "density") +
  ggtitle("Risk table for CD20+ density")
rt_fig2c
```
...
fig2e <- lstat_boxplot(l, cell = "CD20+", label.y = 1)
fig2e
```
### Save output
...
```{r}
rt_fig2f <- faceted_risktable(fig2f, faceted.by = "delta_l") +
  ggtitle("Risk table for delta L: CD20+")
rt_fig2f
```
...
```{r}
rt_fig4d2 <- faceted_risktable(fig4d2, faceted.by = "density") +
  ggtitle("Risk table for CD3+CD8+ density")
rt_fig4d2
```
...
```{r}
fig4e <- lstat_boxplot(l, cell = "CD3+CD8+", label.y = 0.25)
fig4e
```
...
```{r}
fig4e2 <- lstat_km(l, cell = "CD3+CD8+", event_type = "OS", returndata = F)
fig4e2
```
...
```{r}
rt_fig4e2 <- faceted_risktable(fig4e2, faceted.by = "delta_l") +
  ggtitle("Risk table for CD3+CD8+ delta L")
rt_fig4e2
```
...
```{r}
pdf(file = here(params$fig4e), width = 12, height = 7)
fig4e
fig4e2
rt_fig4e2
dev.off()
...
fig4f <- lcross_boxplot(lximmune, cell.1 = "CD3+CD8+", cell.2 = "PanCK+",
...
fig4f2 <- lcross_km(lximmune, cell.1 = "CD3+CD8+", cell.2 = "PanCK+", event_type = "OS", returndata = F)
fig4f2
```
...
```{r}
rt_fig4f2 <- faceted_risktable(fig4f2, faceted.by = "delta_l") +
  ggtitle("Risk table for delta L: CD3+CD8+ <-> PanCK+")
rt_fig4f2
```
...
```{r}
pdf(file = here(params$fig4f), width = 12, height = 7)
fig4f
fig4f2
rt_fig4f2
dev.off()
...
```{r}
fig4g <- lcross_boxplot(lximmune, cell.1 = "CD3+CD8+", cell.2 = "CD3+CD8-",
...
```{r}
fig4g2 <- lcross_km(lximmune, cell.1 = "CD3+CD8+", cell.2 = "CD3+CD8-",
...
```{r}
rt_fig4g2 <- faceted_risktable(fig4g2, faceted.by = "delta_l") +
  ggtitle("Risk table for delta L: CD3+CD8+ <-> CD3+CD8-")
rt_fig4g2
```
...
```{r}
pdf(file = here(params$fig4g), width = 12, height = 7)
fig4g
fig4g2
rt_fig4g2
dev.off()
...
```{r}
fig4h <- lcross_boxplot(lximmune, cell.1 = "CD20+", cell.2 = "CD3+CD8+",
...
```{r}
fig4h2 <- lcross_km(lximmune, cell.1 = "CD20+", cell.2 = "CD3+CD8+",
                    event_type = "OS", returndata = F)
fig4h2
```
...
```{r}
rt_fig4h2 <- faceted_risktable(fig4h2, faceted.by =  "delta_l") +
  ggtitle("Risk table for delta L: CD20+ <-> CD3+CD8+")
rt_fig4h2
```
...
```{r}
pdf(file = here(params$fig4h), width = 12, height = 7)
fig4h
fig4h2
rt_fig4h2
dev.off()
...
```{r}
fig4i <- lcross_boxplot(lximmune, cell.1 = "CD20+", cell.2 = "CD3+CD8-",
...
```{r}
fig4i2 <- lcross_km(lximmune, cell.1 = "CD20+", cell.2 = "CD3+CD8-",
                    event_type = "OS", returndata = F)
fig4i2
```
...
```{r}
rt_fig4i2 <- faceted_risktable(fig4i2, faceted.by =  "delta_l") +
  ggtitle("Risk table for delta L CD20+ <-> CD3+CD8-")
rt_fig4i2
```
...
```{r}
pdf(file = here(params$fig4i), width = 12, height = 7)
fig4i
fig4i2
rt_fig4i2
dev.off()
