...
```{r}
ciber_input = read_csv(here("results", "cibersortX", "CIBERSORTx_Job2_Results.csv"))
...
ciberdf = ciber_input %>%
  ...
  left_join(., select(meta, sample_name, study_group,
                      ...
                      stage, grade,ki67, involution_months,
                      breastfeeding_months), by = "sample_name")
...
For this we need an updated column from a new metadata file. Use Months_Involution_Breastfeeding to incorporate the time between diagnosis and pregnancy as well as involution.

```{r}
meta2 = readxl::read_excel(here("data", "external", "Survival_Hecoderingslijst HL-FA_v29072019_KM.xlsx"))

meta2 = meta2 %>% dplyr::mutate(
    patient_ref = str_replace_all(`NEW REF`, "_", "-"),
    old_ref    = str_replace_all(`OLD REF`, "_", "-"),
    sample_ref = patient_ref)
meta2 = meta2 %>% mutate(study_group = str_replace_all(tolower(str_trim(`Study Group`, side="both")), "-", "_")) %>%
  mutate(sample_name = paste(study_group, str_remove(string = patient_ref, pattern = "HL-FA-"), sep="_")) %>%
  select(sample_name, study_group, everything())



ciberdf %>% inner_join(.,select(meta2, sample_name, Months_Involution_Breastfeeding), by = "sample_name")
```

Ensure that there are no samples lost from the inner join. It looks like when joining on sample names, one sample is lost because it was marked as prbc in the old metadata file and involution in the new metadata file. This case should in fact be involution.

```{r}
setdiff(meta$sample_name, meta2$sample_name) #prbc_38
meta %>% filter(sample_name == "prbc_38") %>% pull(study_group)
meta2 %>% filter(patient_ref == "HL-FA-38") %>% pull(study_group)
```

However all the patient refs from both metadata files are the same.

```{r}
stopifnot(length(setdiff(meta$patient_ref, meta2$patient_ref))==0)
```

Define short as less than 1 month and long as greater than 1 month (999 = unknown)

```{r}
ciberdf = ciberdf %>% 
  mutate(newstudygroup = case_when(
    study_group == "ppbc_inv" & breastfeeding_months == 999 ~ "unknownBFinv", 
    study_group == "ppbc_inv" & breastfeeding_months != 999 & breastfeeding_months <= 1 ~ "shortBFinv",
    study_group == "ppbc_inv" & breastfeeding_months != 999 & breastfeeding_months > 1 ~ "longBFinv",
    TRUE ~ study_group
