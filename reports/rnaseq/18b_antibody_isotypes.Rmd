...
igNorm <- normTMMlog2(dgeIg) %>%
  ...
  left_join(., select(dgeIg$samples, sample_name, study_group, PPBC,
                      ...
                      age_diagnosis, PAM50, 
                      grade, stage = stage_short_integer, surgery,
...
#OS
survfit(Surv(time=months_overall_survival, event=overall_survival) ~ IgA_rank,
...
#DRS
survfit(Surv(time=months_to_drs, event=distant_recurrence) ~ IgA_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
...
```{r, fig.width=6, fig.height=4}
survfit(Surv(time=months_overall_survival, event=overall_survival) ~ IgD_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Overall survival probability",
...
#DRS
survfit(Surv(time=months_to_drs, event=distant_recurrence) ~ IgD_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Distant recurrence probability",
...
```{r, fig.width=6, fig.height=4}
survfit(Surv(time=months_overall_survival, event=overall_survival) ~ IgG_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Overall survival probability",
...
#DRS
survfit(Surv(time=months_to_drs, event=distant_recurrence) ~ IgG_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Distant recurrence probability",
...
```{r, fig.width=6, fig.height=4}
survfit(Surv(time=months_overall_survival, event=overall_survival) ~ IgM_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Overall survival probability",
...
#DRS
survfit(Surv(time=months_to_drs, event=distant_recurrence) ~ IgM_rank,
        data = as.data.frame(igSurv)) %>% 
  ggsurvplot(fit = ., facet.by = "PPBC",
             xlab = "Months", 
             ylab = "Distant recurrence probability",
...
```{r}
coxph(Surv(time=months_overall_survival,
           event=overall_survival) ~ IgA + IgD + IgG + IgM,
      data = igSurv) %>%
  summary()
```
