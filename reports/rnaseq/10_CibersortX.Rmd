...
params:
  ...
  rel_hm: "results/rnaseq/cibersortX/cibersort_relative_heatmap.pdf"
---
...
rel_hm = Heatmap(hmat[rownames(hmat)!="T cells regulatory (Tregs)",],
                 top_annotation = colTop, show_column_names = F,
                 column_title = "Overview CibersortX, k-means clustering (exclude Tregs)",
                 heatmap_legend_param = list(title="Row-scaled relative fraction",
                                             legend_direction = "horizontal"),
                 column_km = 3, row_km = 3, column_km_repeats = 10, row_km_repeats = 10)

draw(rel_hm,  heatmap_legend_side = "bottom")
```
...
#Matrix from df
hmat = abs_ciber_input %>%
  select(-`P-value`, -Correlation, -RMSE, -`Absolute score (sig.score)`) %>%
  column_to_rownames("Mixture") %>% as.matrix %>% t()
hmat = rowScales(hmat)

hm_abs = Heatmap(hmat[rownames(hmat)!="T cells regulatory (Tregs)",],
                 top_annotation = colTop, show_column_names = F,
                 column_title = "Overview CibersortX, k-means clustering (exclude Tregs)",
                 heatmap_legend_param = list(title="Row-scaled absolute fraction",
                                             legend_direction = "horizontal"),
                 column_km = 3, row_km = 3, column_km_repeats = 10, row_km_repeats = 10)

draw(hm_abs,  heatmap_legend_side = "bottom")
```
