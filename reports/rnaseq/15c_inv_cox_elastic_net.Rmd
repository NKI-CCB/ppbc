---
title: "Cox elastic net, all genes, involution only"
author: "Kat Moore"
date: "`r Sys.Date()`"
output: 
  html_document:
...
As report 15b, but with all genes.
...
#Ensure matrix class
survdata <- data.matrix(survdata)
survdata.drs <- data.matrix(survdata.drs)
```
...
glm.os.all <- readRDS(file.path(resDir,"15c_inv_glm_os_all.Rds"))

glm.drs.all <- readRDS(file.path(resDir,"15c_inv_glm_drs_all.Rds"))

glm_list <- list(
  os.all=list(model=glm.os.all,
              title="Overal survival, all genes"),
  drs.all=list(model=glm.drs.all,
               title="Distant recurrence, all genes")
)
```
...
par(mar=c(4,4,2,2))
par(mfrow=c(5,2))
lapply(glm.os.all$fits[1:10], plot)
```
...
set.seed(123)
#survdata[1:10,1:10]
glm.os.zoom = cv.glmnet(x = survdata, y = time_os, family = "cox", alpha = 0.5, df=100)
plot(glm.os.zoom)
```
...
par(mar=c(4,4,2,2))
par(mfrow=c(5,2))
lapply(glm.drs.all$fits[1:10], plot)
```
...
tibble(
  ...
  outcome = c("survival", "metastasis"),
  input_var_genes = c("all", "all"),
...
```{r, include=F}
pdf(here("results", "survival", "15c_hist_features_cox_enet.pdf"), height=6, width=9)
...
find_feature_haplotypes(glm.features$os.all$df)
```

Volcano-like plot, with and without clinical variables:

```{r, fig.height=10, fig.width=7}
ggarrange(feature.volc$os.all$clin,
          feature.volc$os.all$no_clin,
          nrow = 2, ncol = 1)
```
...
```{r}
glm.features$drs.all$df
```

Some genes go from positive to negative beta, depending on the haplotype: 

```{r}
find_feature_haplotypes(glm.features$drs.all$df)
```

Volcano-like plot, with and without clinical variables:

```{r, fig.height=10, fig.width=7}
ggarrange(feature.volc$drs.all$clin,
          feature.volc$drs.all$no_clin,
          nrow = 2, ncol = 1)
```
...
```{r, fig.height = 10, fig.width=7}
fmat = as.matrix(feature_overlap[1:50,2:3])
...
relplots = lapply(list(relplot.df$os.all), function(x){
  erplot.df = x %>%
...
lapply(list(relplot.df$drs.all), function(x){
  erplot.df = x %>%
...
```{r}
ggsave(here("results", "survival", "15c_cox_predictions.pdf"), height=6, width = 9)
```
...
pears_cor = 
tibble(
  cor = c(signif(cor(relplot.df$os.all$rel.risk, relplot.df$os.all$overall_survival,
          use="complete.obs", method = "pearson"), 2),
          signif(cor(relplot.df$drs.all$rel.risk,
                     as.numeric(as.character(relplot.df$drs.all$distant_recurrence)),
          use = "complete.obs", method = "pearson"),2)
  ),
  outcome = c("overall survival", "distant recurrence"),
  input = c(rep("all genes", 2)),
  method = rep("Pearson", 2),
...
spear_cor = spear_cor %>%
  mutate(n = unlist(lapply(list(time_os, time_drs), nrow))) %>%
  rowwise() %>%
...
openxlsx::write.xlsx(lapply(glm.features, function(x){x[[1]]}),
                     file = here("results", "survival", "15c_all_inv_elastic_cox_features.xlsx"))
write_csv(spear_cor, here("results/survival/15c_enetcox_pvals.csv"))
```
