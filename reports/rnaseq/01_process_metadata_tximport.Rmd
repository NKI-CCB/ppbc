...
colnames(meta_raw)
```
How many of these names changed since the previous version?

```{r}
meta_old = readxl::read_excel(here("data", "external", "Hecoderingslijst HL-FA_v29072019_KM.xlsx"))

table(colnames(meta_raw) %in% colnames(meta_old))
```
Substantial changes between this version and the previous means we will need to carefully check the columns.
...
# Clean PPBC group names

For when we prefer full words over acronyms (plotting).
...
sample_annot = sample_annot %>%
  ...
  select(sample_id:study_group, PPBC, everything())
