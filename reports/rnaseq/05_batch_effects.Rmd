...
pca.df$patient_ID <- rownames(nc.pca$x)
pca.df <- inner_join(pca.df, sd, by='patient_ID')
pca.df <- pca.df %>% select(patient_ID, batch, everything())
head(pca.df)
```
...
```{r kw-test}
batch <- sd[match(colnames(normcounts), sd$patient_ID),]$batch
...
pca.df.nl$patient_ID <- rownames(nc.pca.nolac$x)
pca.df.nl <- inner_join(pca.df.nl, sd, by='patient_ID')
pca.df.nl = pca.df.nl %>% select(patient_ID, batch, everything())
#head(pca.df.nl)
```
...
```{r}
sd <- sd[match(sd$patient_ID, colnames(dds)),]

stopifnot(all(sd$patient_ID==colnames(dds)))
stopifnot(all(sd$patient_ID==colData(dds)$patient_ID))
