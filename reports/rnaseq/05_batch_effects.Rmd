...
Adapted for the post-partum breast cancer project (ppbc).
...
dds <- readRDS(file=here("data/Rds/04_dds_PAM50_EST.Rds"))
...
Very small batches are going to cause problems in differential expression later on if batch is used as part of the design formula.
To handle this, we combine batches to ensure that they are at least 10 samples in size.
...
```{r, include=F}
resdir = here("results", "PCA")
dir.create(resdir, showWarnings = F)
ggsave(filename = file.path(resdir, "05_scree.pdf"),
...
```{r, include=F}
ggsave(filename=file.path(resdir, "05_sigPCA_batch.pdf"),
...
```{r, include=F}
ggsave(filename=file.path(resdir, "05_firstPCA_batch.pdf"),
       device="pdf", width=8, height=5)
```
...
```{r, include=F}
ggsave(filename=file.path(resdir, "05_sigPCA_PPBC.pdf"),
       device="pdf", width=8, height=5)
```
...
```{r, include=F}
ggsave(filename=file.path(resdir, "05_firstPCA_PPBC.pdf"),
       device="pdf", width=8, height=5)
```
...
If we do this, only PC13 remains significant.
...
PC_study_nolac = lapply(nl, function(x) x$p.value) %>% unlist() %>% enframe("PC", "nom.p") %>%
  mutate(bonferroni=p.adjust(nom.p, "bonferroni"), fdr = p.adjust(nom.p, "BH")) %>%
  filter(fdr < 0.05) %>% arrange(fdr)

print(paste("There are", nrow(PC_study_nolac), "principal components significantly associated with study group."))
...
```{r, include=F}
ggsave(filename=file.path(resdir, "05_sigPCA_Pam50.pdf"),
       device="pdf", width=8, height=5)
```
...
We can alternatively visualize this as a traditional heatmap:
...
The overall amount of variance associated with the principal components is relatively low (~9% for PC1). 
However, around 12K/60K genes are significant in a kruskal-wallis test of gene expression vs batch. 
16 out of the first 100 principal components are significantly associated with batch. 
This suggests that include batch in the design formula for differential expression analysis is prudent.

More problematically, only 2 PCs are associated with study group. 
A chisquare analysis indicates a strong association between batch and study group.
This makes the inclusion of batch in the design formula less effective as a compensation mechanism.
However, extensive experimentation with alternate design formulas shows that excluding batch from the design does not improve the outcome.
See notebook 17 in 6 April 2020 commit: 524ee07dc463359ed4566817bef927694bd4daba.

As predicted, PAM50 is strongly associated with most of the first 10 principal components.
However, tumor purity is not associated with any of the first 100 PCs.
This suggests that it will yield little additional benefit if included in the design formula, and may in fact over-complicate the design.
...
dds$study_group = factor(dds$study_group,
                         levels = c("non_prbc", "prbc",
                                    "ppbc_lac", "ppbc_inv"))
