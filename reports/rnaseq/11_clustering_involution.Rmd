...
```{r}
write_csv(IG_kaplan_PAM50_OS$data, here(params$rtIG_kaplan_PAM50_OS))
```
...
rtIG_kaplan_PPBC_OS <- risktable(IG_kaplan_PPBC_OS, response = "IG", nrow=3)
rtIG_kaplan_PPBC_OS
```
...
```{r}
write_csv(IG_kaplan_PPBC_OS$data, here(params$rtIG_kaplan_PPBC_OS))
```
...
```{r}
write_csv(IG_kaplan_PAM50_DRS$data, here(params$rtIG_kaplan_PAM50_DRS))
```
...
```{r, fig.height=7, fig.width=12}
rtIG_kaplan_PPBC_DRS <- risktable(IG_kaplan_PPBC_DRS, response = "IG", nrow=3)
...
```{r}
write_csv(IG_kaplan_PPBC_DRS$data, here(params$rtIG_kaplan_PPBC_DRS))
```
