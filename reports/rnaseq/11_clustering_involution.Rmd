...
pdf(file = here(params$fig1a), width = 10, height = 10)
fig.hm
dev.off()
```
...
```{r, fig.width=8}
nolacsurv <- as.data.frame(droplevels(filter(surv, study_group != "ppbcdl")))
...
ig_km <- ggsurvplot(
    ...
    pval = T, 
    facet.by = "IG",
    palette = study_colors[!names(ppbc_colors) %in% c("ppbcdl", "rest")],
    ggtheme = theme_bw(),
    risk.table = T)
...
Risk table is extremely hard to plot legibly.

```{r, fig.height=7, fig.width=15}
text.size <- 8

rt_ig_km <- ggplot(ig_km$data, aes(time, study_group)) + 
  #geom_text(aes(label = n.risk), position=position_jitter(width = 0, height = 0.1)) +
  ggrepel::geom_text_repel(aes(label = n.risk)) +
  facet_wrap(~IG, 3) + theme_bw() + 
...
```{r}
write_csv(ig_km$data, here(params$risktable1e))
```
...
coxph(
  Surv(time=time_OS_months,
       event=death) ~ age_at_diagnosis + grade + stage + surgery + radiotherapy + strata(hormonetherapy) + chemotherapy + herceptin + PAM50 + IG + study_group,
  data = surv) %>%
...
coxph(
  Surv(time=time_OS_months,
       event=death) ~ age_at_diagnosis + year_diagnosis + grade + stage + surgery + radiotherapy + strata(hormonetherapy) + chemotherapy + herceptin + PAM50 + strata(IG) + study_group,
  data = surv) %>%
...
```{r, fig.width=10}
ggsurvplot(
    fit = survfit(Surv(time=time_DRS_months, event=distant_recurrence) ~ PPBC + IG, data = surv), 
    xlab = "Months", 
    ylab = "DRS probability",
...
coxph(
  Surv(time=time_DRS_months,
       event=distant_recurrence) ~ age_at_diagnosis + year_diagnosis + grade +
    stage + surgery + radiotherapy + strata(hormonetherapy) + chemotherapy +
    herceptin + PAM50 + strata(IG) + PPBC,
  data = surv) %>%
  ggadjustedcurves(., variable = "IG", data = as.data.frame(surv), method="conditional", 
                   palette = cp$tertile_colors,
    ggtheme = theme_bw()) +
