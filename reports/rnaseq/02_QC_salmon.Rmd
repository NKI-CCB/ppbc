...
```{r}
refseq_db <- read_tsv(here(params$refseq_db), show_col_types = F)
...
multiqc <- multiqc %>%
  mutate(poor_mapping = percent_mapped < 75, .after = percent_mapped)
...
as.data.frame(qc) %>% filter(module=="Per tile sequence quality") %>%
  left_join(., multiqc, by = c("sample"="fastq")) %>%
