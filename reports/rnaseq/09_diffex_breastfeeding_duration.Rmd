---
title: "Diffex Breastfeeding Duration"
Author: Kat Moore
date: "`r Sys.Date()`"
output: 
  html_document:
...
dds <- dds[,!is.na(dds$breastfeeding_duration)]
```
...
```{r}
dds$breastfeeding_duration = 
  recode(dds$breastfeeding_duration,
       "<= 1 month"="1.mo.or.less",
       "> 1 month"="more.than.1.mo")

dds$breastfeeding_duration = 
  relevel(dds$breastfeeding_duration,
        ref="more.than.1.mo")
...
table(dds$PPBC, dds$breastfeeding_duration)
table(dds$batch, dds$breastfeeding_duration)
table(dds$PAM50, dds$breastfeeding_duration)
```

Discuss: Combine small groups?

```{r}
design(dds) <- ~batch + PAM50 + breastfeeding_duration
```

# Differential expression

```{r DESeq}
ddsfile = here("data/Rds/09_dds_breastfeeding_duration.Rds")
overwrite = F
...
```{r}
apefile = here("data/Rds/09_ape_breastfeeding_duration.Rds")

if(file.exists(apefile) == F | overwrite == T){
  tic()
  ape = lfcShrink(dds, type = "apeglm", res=rdds,
                coef = "breastfeeding_duration_1.mo.or.less_vs_more.than.1.mo")
  toc()
  saveRDS(ape,apefile)
} else {
  ape = readRDS(apefile)
...
```{r}
ts = "Breastfeeding duration: <=1mo vs >1mo"
```
...
Few results: due to l2fc threshold?
...
We'd only pick up one more hit if we relazed the l2fc threshold.
...
names(bf_colors) <- c("1.mo.or.less", "more.than.1.mo")
#levels(dds$breastfeeding_duration)
```
...
deseq_heatmap = function(mat = geneEx,
                         ...
                         sig_res = sig_results,
                         groups_to_plot=levels(as.data.frame(colData(dds))[,"breastfeeding_duration"]),
                         title= "Involution duration",
                         #intgroup = "breastfeeding_duration",
                         top_vars = "breastfeeding_duration",
                         top_colors = list(breastfeeding_duration=bf_colors),
                         bottom_vars = "death",
...
plot_gene_simple = function(dds, ensembl_gene_id, gene_name,
                            intgroup="breastfeeding_duration", colorby="death"){
  require(DESeq2)
  require(tidyverse)
...
write.xlsx(resdf, file = here("results", "diffex", "09_diffex_breastfeeding_duration.xlsx"))
rm(resdf)
```
...
```{r}
resDir = here("results", "diffex", "figs", "09_breastfeeding_duration")
dir.create(resDir, showWarnings = F)

pdf(file.path(resDir, "09_hm_breastfeeding_duration.pdf"), width = 10, height = 8)
...
```{r}
ggsave(filename=file.path(resDir,"09_volcano_breastfeeding_duration.jpeg"),
