...
Hanne confirms that this sample is ER positive.

```{r}
dds$ER[is.na(dds$ER)] <- 1
```
...
```{r}
vsub %>%
  ggplot(aes(x=molecular_subtype, y = normCount)) +
  geom_boxplot() +
...
```{r}
write_csv(ihc_outlier, path = here("data/metadata/03_ihc_outliers.csv"))
```
...
table(pred$subtype)
```
Verify that nothing has changed vs the previous metadata:
...
Discordance is greatest for LumB Her2+ samples: They end up spread between Her2, LumB and LumA.
...
vpam = vroi %>% as.data.frame() %>% rownames_to_column("gene_id") %>%
  ...
  left_join(.,
            select(as.data.frame(colData(dds)),
                   sample_name, HER2, ER, PR, PAM50, molecular_subtype),
            by= "sample_name")
...
```{r}
write_csv(pam50_outliers, path = here("data/metadata/03_removed_pam50_outliers.csv"))
```
