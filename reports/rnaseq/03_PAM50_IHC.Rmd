...
vsub <- vroi %>% as.data.frame() %>% rownames_to_column("gene_id") %>%
  gather(key = "patient_ID",-gene_id, value = "normCount") %>%
  left_join(roi, ., by="gene_id") %>%
  left_join(.,select(sd, patient_ID, HER2, ER, PR, clin_subtype), by= "patient_ID")
...
plot_receptor(vsub, "HER2") +
  geom_text_repel(aes(label=patient_ID), show.legend = F,
            data=her2_outlier)
```
...
```{r}
ihc_outlier <-
  right_join(select(sd, patient_ID, HER2, ER, PR),
             bind_rows(
               tibble(patient_ID = her2_outlier$patient_ID,
                      ihc_outlier = "HER2_high"),
               tibble(patient_ID=er_outlier$patient_ID,
                      ihc_outlier = "ER_high"),
               tibble(patient_ID=pr_outlier$patient_ID,
                      ihc_outlier = "PR_high")),
             by="patient_ID")
...
```{r}
stopifnot(identical(colData(dds)[,"patient_ID"], names(pred$subtype)))
...
vpam <- vroi %>% as.data.frame() %>% rownames_to_column("gene_id") %>%
  ...
  left_join(.,
            select(as.data.frame(colData(dds)),
                   patient_ID, HER2, ER, PR, PAM50, clin_subtype),
            by= "patient_ID")
...
get_max_prob <- function(prob_matrix, samples=rownames(prob_matrix)){
  ...
  df = tibble(patient_ID = names(max_prob),
              prediction = colmax,
...
stopifnot(identical(prob_pred$patient_ID,dds$patient_ID))
...
```{r}
left_join(prob_pred, vpam, by="patient_ID") %>%
...
```{r}
plot_receptor(left_join(prob_pred, vpam, by="patient_ID"), "HER2", x.column="PAM50") +
...
pam50_outliers <- filter(vpam, PAM50 == "Basal" & gene_name=="HER2" & normCount>14) %>%
  select(patient_ID, HER2, ER, PR, PAM50) %>% mutate(pam50_outlier = "highHER2_tripleneg")
pam50_outliers
```
...
```{r}
plot_receptor(left_join(prob_pred, vpam, by="patient_ID"), "ER", x.column="PAM50") +
...
```{r}
plot_receptor(left_join(prob_pred, vpam, by="patient_ID"), "PR", x.column="PAM50") +
...
pam50_outliers <- bind_rows(pam50_outliers, 
  ...
  select(patient_ID, HER2, ER, PR, PAM50) %>% mutate(pam50_outlier = "highPR_tripleneg"))
pam50_outliers
```
...
```{r}
pam50_outliers$patient_ID %in% ihc_outlier$patient_ID
```
...
dds = dds[,!colnames(dds) %in% pam50_outliers$patient_ID]
