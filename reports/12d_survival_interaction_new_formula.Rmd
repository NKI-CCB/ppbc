...
print(paste("Total number of OS-associated genes with fdr < 0.25:", nrow(os[os$fdr < 0.25,])))
...
print(
  paste(
    "Total number of OS-associated genes with fdr < 0.1:",
    (os %>% filter(fdr < 0.1) %>% nrow())
  ))

os %>% filter(fdr < 0.1) %>% pull(gene_name) %>% paste(collapse = ", ")
...
p %>%
  ...
  ggtitle("Interaction model for overall survival")
```

```{r}
os %>% arrange(beta) %>% head(1)
```
...
p = drs %>% 
  mutate(type=case_when(
        ...
        gene_type == "protein_coding" ~ "protein coding",
        TRUE ~ "other noncoding")) %>%
  mutate(type = ifelse(fdr > 0.2, NA, type))
...
p %>%
  ...
  ggthemes::theme_few() +
  ggthemes::scale_color_few(na.value = "grey50")+
  ggrepel::geom_label_repel(show.legend = F, data = filter(p,fdr < 0.2 & type == "protein coding")) +
  ggtitle("Interaction model for distant recurrence")
```

```{r}
drs %>% arrange(beta) %>% head(1)
```
