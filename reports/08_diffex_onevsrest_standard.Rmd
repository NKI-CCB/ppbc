...
surv_colors <- readRDS(here("data", "Rds", "17_survival_colors.Rds"))
os_colors <- surv_colors$os_colors
drs_colors <- surv_colors$drs_colors
```
...
Per request, add code which will allow us to add survival categories to the heatmaps.
...
saveRDS(dds, here("data/Rds/08_dds_standard_vs_rest.Rds"))
```
...
saveRDS(vsd, here("data", "Rds", "08_vsd_standard_vsrest.Rds"))
```
...
rep_inv_rest = deseq_report(ape_inv_rest, dds=dds, absl2fc = 0.5, groups = c("ppbc_inv", "rest"),
                            top_vars = c("study_group", "survival"),
