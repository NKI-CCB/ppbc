...
Common milk protein, upregulated in pregnancy. Sometimes subject to Cook's distance filtering due to extreme outliers.

```{r}
gene_results(res_list, "ENSG00000167531")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000167531",])
```
...
```{r}
gene_results(res_list, "ENSG00000183206")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000183206",])
```
...
```{r}
gene_results(res_list, "ENSG00000166351")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000166351",])
```
...
```{r}
gene_results(res_list, "ENSG00000265766")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000265766",])
```
...
A good example of a gene which "looks" significant but is reported with a lower log2fc and higher p value than the graph would indicate.
...
```{r}
gene_results(res_list,"ENSG00000140009")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000140009",])
```

## [FOXP3](https://www.genecards.org/cgi-bin/carddisp.pl?gene=FOXP3&keywords=foxp3)

```{r}
gene_results(res_list,"ENSG00000049768")
```

```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000049768",])
```
...
```{r}
plot_gene_beehive(dds, rep_inv_rest$annotated_results[rep_inv_rest$annotated_results$ensembl_gene_id=="ENSG00000169083",])
```
