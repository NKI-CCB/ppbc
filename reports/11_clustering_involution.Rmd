...
deseq_heatmap(mat = geneEx, sampledata = mutate(sd, study_group = study_group_orig), 
              top_colors = study_colors, bottom_colors = pam_colors,
...
#as.hclust(coldend.hm[[1]])$labels %>% unlist(use.names = F)
```
...
```{r}
#surv$death
survdiff(survival::Surv(time =months_overall_survival, event = death) ~ IG_cluster, data=surv)

```

Multivariate (other covariates?)

```{r}
coxph(survival::Surv(time =months_overall_survival, event = death) ~ PAM50 + IG_cluster, data=surv) #%>%
  #summary()
```
```{r}
ggsurvplot(
    fit = survfit(Surv(time=months_overall_survival, event=death) ~ IG_cluster, data = surv), 
    xlab = "Months", 
    ylab = "Overall survival probability",
    title = "All study groups",
    pval = T)
...
```{r}
surv.inv = surv %>% filter(study_group == "ppbc_inv")

survdiff(survival::Surv(time =months_overall_survival, event = death) ~ IG_cluster, data=surv.inv)
```
...
```{r}
ggsurvplot(
    fit = survfit(Surv(time=months_overall_survival, event=death) ~ IG_cluster, data = surv.inv), 
    xlab = "Months", 
    ylab = "Overall survival probability",
    title= "Involution patients",
    pval = T)
```
...
```{r, fig.width=10}
  ...
  ggsurvplot(
    fit = survfit(Surv(time=months_overall_survival, event=death) ~ PAM50 +IG_cluster, data = surv.inv), 
    xlab = "Months", 
    ylab = "Overall survival probability",
    title = "All study groups",
    pval = T, facet.by = "PAM50")
...
survdiff(survival::Surv(time =months_to_drs, event = dist_recurrence_free_survival) ~ IG_cluster, data=surv.inv)

```

```{r}
ggsurvplot(
    fit = survfit(Surv(time=months_to_drs, event=dist_recurrence_free_survival) ~ IG_cluster, data = surv.inv), 
    xlab = "Months", 
    ylab = "DRS",
    title= "All patients",
    pval = T)
...
survdiff(survival::Surv(time =months_to_lrs, event = local_recurrence_free_survival) ~ IG_cluster, data=surv.inv)

```

```{r}
ggsurvplot(
    fit = survfit(Surv(time=months_to_lrs, event=local_recurrence_free_survival) ~ IG_cluster, data = surv.inv), 
    xlab = "Months", 
    ylab = "LRS",
    title= "Involution only",
    pval = T)
```
...
```{r, eval=F}
library(rmarkdown)
#Puts a copy in the doc folder
render(here("reports", "11_clustering_involution.Rmd"),
       output_dir = here("doc"))
```
