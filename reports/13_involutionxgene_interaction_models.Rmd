...
params:
  survival_results:  
    value: "/DATA/share/postpartumbc/data/Rds/13_uni_interaction_os.Rds"
---
...
Ensembl df for box plots:

```{r}
ensEx = select(coxdata, ENSG00000000003:ncol(coxdata))
ensEx = rownames_to_column(as.data.frame(t(ensEx)), "ensembl_gene_id")
ensEx = ensEx %>%
  gather(key = "sample_name", -ensembl_gene_id, value = "logTMM")
head(ensEx)
```
...
Skip if less than 30 genes significant at given threshold:

```{r}
if(nrow(filter(res, fdr < !!pthresh & beta > 0)) < 30){
  delpaths = NULL
  print("Fewer than 30 significant genes, skipping pathway analysis")
} else {
...
gene_data = rownames_to_column(coxdata[,genecol:ncol(coxdata)], "sample_name")
#head(sample_data)

box_plots <- function(id, gEx = gene_data, d=dds, s = sample_data, cox_res = res){
  
  gene_res = cox_res %>% filter(ensembl_gene_id == !!id)
  gn = gene_res$gene_name 
  fdr = gene_res$fdr
  beta = gene_res$beta
  title = paste0(gn, ":", id,
                 ", fdr: ", signif(fdr,2),
