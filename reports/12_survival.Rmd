...
saveRDS(vsd, here("data","Rds","12_vst_mat_gene_symbol.Rds"))
```
...
```{r}
openxlsx::write.xlsx(metadata, file = here("data", "metadata", "12_survival_metadata.xlsx"))
```
...
```{r}
bf_coxdata <- as.data.frame(colData(dds)) %>% 
  ...
  select(sample_name, newstudygroup) %>%
  right_join(., coxdata, by = "sample_name")

bf.clin.cov.cox <- coxph(
  Surv(time=months_overall_survival,
       event=overall_survival) ~ age_at_diagnosis + year_of_diagnosis + grade + stage + surgery + radiotherapy + hormonetherapy + chemotherapy + herceptin + PAM50 + newstudygroup,
  data = bf_coxdata)
summary(bf.clin.cov.cox)
...
bf_strata <- coxph(
  Surv(time=months_overall_survival,
       event=overall_survival) ~ age_at_diagnosis + year_of_diagnosis + grade + stage + surgery + radiotherapy + hormonetherapy + chemotherapy + herceptin + PAM50 + strata(newstudygroup), data = bf_coxdata)

ggadjustedcurves(bf_strata, data=bf_coxdata, method="conditional", variable="newstudygroup") +
  ggtitle("Adjusted survival curve for clinical covariates: Stratified")

ggadjustedcurves(bf.clin.cov.cox, variable = "newstudygroup", data = bf_coxdata, method="conditional") +
  ggtitle("Adjusted survival curve for clinical covariates: Balanced")
...
```{r}
ggadjustedcurves(drs.clin.cov.cox, variable = "study_group", data = coxdata, method="conditional") +
  ggtitle("Adjusted drs curve for clinical covariates: Balanced") +
  ylab("DRS rate")
```
...
bf.drs.clin.cov.cox <- coxph(
  Surv(time=months_to_drs,
       event=distant_recurrence) ~ age_at_diagnosis + year_of_diagnosis + grade + stage + surgery + radiotherapy + hormonetherapy + chemotherapy + herceptin + PAM50 + newstudygroup,
  data = bf_coxdata)
summary(bf.drs.clin.cov.cox)
...
bf_drs_strata <- coxph(
  Surv(time=months_to_drs,
       event=distant_recurrence) ~ age_at_diagnosis + year_of_diagnosis + grade + stage + surgery + radiotherapy + hormonetherapy + chemotherapy + herceptin + PAM50 + strata(newstudygroup), data = bf_coxdata)

ggadjustedcurves(bf_drs_strata, data=bf_coxdata, method="conditional", variable="newstudygroup") +
  ggtitle("Adjusted DRS for clinical covariates: Stratified")

ggadjustedcurves(bf.drs.clin.cov.cox, variable = "newstudygroup", data = bf_coxdata, method="conditional") +
  ggtitle("Adjusted DRS curve for clinical covariates: Balanced")
...
p = res_allgenes_multivar_surv %>% 
  ...
  mutate(type = ifelse(fdr > 0.1, NA, type))

p %>%
  ggplot(aes(x = beta, y = -log10(fdr), color = type, label = gene_name)) +
  geom_point(alpha = 0.8) +
  geom_hline(yintercept = -log10(0.1), linetype = "dashed") +
  ggthemes::theme_few() +
  ggthemes::scale_color_few(na.value = "grey50")+
  ggrepel::geom_label_repel(show.legend = F, data = filter(p,fdr < 0.05 & type == "protein coding" |
                                                             fdr < 0.1 & abs(beta) > 1.5)) +
  ggtitle("Genewise multivariate Cox model for overall survival")
```
...
## OS: Comparing multivariate and univariate
...
p %>%
  ...
  ggthemes::theme_few() +
  ggthemes::scale_color_few(na.value = "grey50")+
  ggrepel::geom_label_repel(show.legend = F, data = filter(p,fdr < 0.1 & type == "protein coding")) +
  ggtitle("Genewise multivariate Cox model for distant recurrence")
```
...
## DR: Comparing multivariate and univariate
...
## Write results: all genes OS and DR
...
meta_allgenes <- tibble(
  ...
  Formula = c("Surv(time=months_overall_survival,event=overall_survival)~gene",
              ...
              "Surv(time=months_to_drs,event=distant_recurrence)~age_at_diagnosis+year_of_diagnosis+stage+grade+surgery+radiotherapy+hormonetherapy+chemotherapy+herceptin+PAM50+gene",
              rep(NA, 5)),
  Package = c(rep("survival/survminer",4), rep(NA, 4), "edgeR"),
  Test = c("Cox survival", "Cox metastasis", "Cox survival", "Cox metastasis", rep("Fisher's exact",4), NA))
...
openxlsx::write.xlsx(list(description = meta_allgenes,
                          ...
                          metadata = as.data.frame(colData(dds))#,
                          #input_norm_geneEx = rownames_to_column(as.data.frame(t(mat)), "ensembl_gene_id") #Unnecessary, makes huge file
                          ),
...
```{r}
plot_cox_beehive(dds, description = "Multivariate DR", ensembl_id = "ENSG00000211898",
                 res_df = lrt, cox_df = res_allgenes_multivar_dr)
```

# Post hoc: Involution samples only

Previously, we looked exclusively at the genes which were significant in involution vs rest.
Now, we'll check to see whether we can identify individual genes which are predictive of survival exclusively in the involution group.
...
```{r}
res_inv_os <- readRDS(here("data","Rds","12_inv_multi_os.Rds")) %>%
  mutate(fdr = p.adjust(p.value, method = "fdr")) %>%
  select(gene_name, fdr, beta, `HR (95% CI for HR)`, everything())
...
```{r, collapse=T}
print(paste("Overall survival Beta > 0, fdr < 0.1:",
      nrow(res_inv_os %>% filter(beta > 0 & fdr < 0.1))))

res_inv_os %>% filter(beta > 0 & fdr < 0.1) %>%
  pull(gene_name)
```


With just 3 genes, we won't be having pathways.

More permissive theresholds:

Beta > 0

```{r, collapse=T}
print(paste("Overall survival Beta > 0, fdr < 0.2:",
      nrow(res_inv_os %>% filter(beta > 0 & fdr < 0.2))))

res_inv_os %>% filter(beta > 0 & fdr < 0.2) %>%
  pull(gene_name)
```
...
```{r}
print(paste("Overall survival multivariate Beta < 0, fdr < 0.2:",
      nrow(res_inv_os %>% filter(beta < 0 & fdr < 0.2))))
res_inv_os %>% filter(beta < 0 & fdr < 0.2) %>%
  pull(gene_name)
```
...
```{r}
#res_inv_os$gene_name %>% length()
path_inv_os = fisher_pathways(
  sig_genes = pull(filter(res_inv_os, fdr < 0.2),gene_name),
      background_genes = res_inv_os$gene_name,
      list_signatures = gene_sets,
      fdr_cutoff = 0.1,
      verbose = F,
      collapse_rows=F)

print("Sig pathways fdr < 0.2: ")

lapply(path_inv_os, nrow)
```

Top of each gene sig:

```{r}
lapply(path_inv_os, function(x) plot_enrichment(enrich_res = x, fdr = 0.1, max_nchar_path = 40, max_path = 20,title = "Sig pathways fdr < 0.2: Multi OS (Inv only)"))
```
...
```{r, fig.height=6, fig.width=9}
bind_rows(path_inv_os, .id = "gene_set")  %>%
...
plot_enrichment(
        ...
        max_nchar_path = 40,
        max_path = 20,
        title = "Sig pathways fdr < 0.2: Multi OS (Inv only)"
      )
```


## Inv samples: Multivariate DRS
...
```{r}
res_inv_drs <- readRDS(here("data","Rds","12_inv_multi_dr.Rds")) %>%
  mutate(fdr = p.adjust(p.value, method = "fdr")) %>%
  select(gene_name, fdr, beta, `HR (95% CI for HR)`, everything())
...
```{r, collapse=T}
print(paste("Distant recurrence Beta > 0, fdr < 0.1:",
      nrow(res_inv_drs %>% filter(beta > 0 & fdr < 0.1))))

res_inv_drs %>% filter(beta > 0 & fdr < 0.1) %>%
  pull(gene_name)
```
...
```{r}
print(paste("Distant recurrence multivariate Beta < 0, fdr < 0.1:",
      nrow(res_inv_drs %>% filter(beta < 0 & fdr < 0.1))))
res_inv_os %>% filter(beta < 0 & fdr < 0.1) %>%
  pull(gene_name)
```

Also here we will not have significant pathways which so few genes.
...
```{r, collapse=T}
print(paste("Distant recurrence Beta > 0, fdr < 0.15:",
      nrow(res_inv_drs %>% filter(beta > 0 & fdr < 0.15))))

res_inv_drs %>% filter(beta > 0 & fdr < 0.15) %>%
  pull(gene_name)
```
...
```{r}
print(paste("Distant recurrence multivariate Beta < 0, fdr < 0.15:",
      nrow(res_inv_drs %>% filter(beta < 0 & fdr < 0.15))))
res_inv_drs %>% filter(beta < 0 & fdr < 0.15) %>%
  pull(gene_name)
```
...
```{r}
#res_inv_os$gene_name %>% length()
path_inv_drs = fisher_pathways(
  sig_genes = pull(filter(res_inv_drs, fdr < 0.15),gene_name),
      background_genes = res_inv_drs$gene_name,
      list_signatures = gene_sets,
      fdr_cutoff = 0.1,
      verbose = F,
      collapse_rows=F)

print("Sig pathways fdr < 0.15: ")

lapply(path_inv_drs, nrow)
```

Top of each gene sig:

```{r}
lapply(path_inv_drs, function(x) plot_enrichment(enrich_res = x, fdr = 0.1, max_nchar_path = 40, max_path = 20,title = "Sig pathways fdr < 0.15: Multi DRS (Inv only)"))
```
...
```{r, fig.height=6, fig.width=9}
bind_rows(path_inv_drs, .id = "gene_set")  %>%
...
plot_enrichment(
        ...
        max_nchar_path = 40,
        max_path = 20,
        title = "Sig pathways fdr < 0.2: Multi DRS (Inv only)"
      )
```
