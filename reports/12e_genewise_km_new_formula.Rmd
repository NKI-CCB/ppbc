...
# Overlap diffex and Cox genes

```{r}
sig_genes <- list(
  "diffex inv vs np" = filter(res_list$inv_vs_nonprbc, padj < 0.05 & abs(log2FoldChange) > 0.5)$ensembl_gene_id,
  "diffex inv vs rest" = filter(res_list$inv_vs_rest, padj < 0.05 & abs(log2FoldChange) > 0.5)$ensembl_gene_id,
  "cox os" = filter(os, fdr < 0.1)$ensembl_gene_id,
  "cox drs" = filter(drs, fdr < 0.1)$ensembl_gene_id,
  "cox inv int os" = filter(inv_int_os, fdr < 0.2)$ensembl_gene_id,
  "cox inv int drs" = filter(inv_int_drs, fdr < 0.2)$ensembl_gene_id  
)
```

```{r, fig.width=8}
upset_surv = UpSetR::upset(UpSetR::fromList(sig_genes), order.by = "freq", #empty.intersections = "on",
                       nsets = 6, set_size.show = TRUE, mainbar.y.label = "Overlap diffex and Cox genes",
...
```{r}
intersect(
  filter(res_list$inv_vs_rest, padj < 0.05 & abs(log2FoldChange) > 0.5)$ensembl_gene_id,
  filter(os, fdr < 0.1)$ensembl_gene_id)
...
```{r, include=F}
gene_summary("CD27")$survival_report %>%
     select(gene_name, type, fdr, p.value, beta, `HR (95% CI for HR)`)
```
...
```{r}
gene_summary("UACA")$survival_report
```
...
```{r, warning=F, fig.height=7, fig.width=10}
km_ntiles_ovr("UACA", ovr_column = "involution", survival_type = "os")
```

```{r, warning=F, eval=F, include=F, fig.height=5, fig.width=10}
#Separated study groups
ggarrange(plotlist = km_ntiles("UACA"), ncol = 2, nrow = 1)
```

### DRS

```{r, warning=F, fig.height=7, fig.width=10}
km_ntiles_ovr("UACA", ovr_column = "involution", survival_type = "drs")
```
...
```{r}
gene_summary("UACA")$diffex_report
```

### Expression

```{r, fig.width=8}
tmm_plots("UACA")$beehive
```
