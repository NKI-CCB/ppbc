...
colData(inv.dds) = droplevels(colData(inv.dds))
...
```{r}
rep_inv_nonprbc = readRDS(here("data", "Rds", "07_report_inv_nonprbc.Rds"))
```

All results

```{r}
res_inv = rep_inv_nonprbc$annotated_results
```
...
```{r}
rep_inv_nonprbc$sig_threshold
```

```{r}
sig.inv = rep_inv_nonprbc$significant_genes
