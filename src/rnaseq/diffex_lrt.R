#Open the bash terminal and log in to server
#Run this command:
...
dds <- readRDS(file = here("data/rnaseq/interim/05b_dds_filtered.Rds"))
...
print("Study group:")
print(levels(dds.nolac$study_group))
dds.nolac$PPBC <- droplevels(dds.nolac$PPBC)
print("PPBC:")
print(levels(dds.nolac$PPBC))

#### Variance stabilizing transformations ####

if(file.exists(here("data","rnaseq","interim","06_vsd.Rds")) == F | overwrite == T){
  print("Variance stabilizing transformation")
  tic()
  vsd = blind_vst(dds)
  saveRDS(vsd, here("data","rnaseq","interim","06_vsd.Rds"))
  toc()
}

if(file.exists(here("data","rnaseq","interim","06_vsd_nolac.Rds")) == F | overwrite == T){
  print("Variance stabilizing transformation")
  tic()
  vsd.nolac = blind_vst(dds.nolac)
  #print(table(vsd.nolac$study_group))
  toc()
  saveRDS(vsd.nolac, here("data","rnaseq","interim","06_vsd_nolac.Rds"))
}


#### Likelihood ratio tests ####

#The likelihood ratio test (LRT) is an ANOVA-like test design to check for genes 
# which are significantly differentially expressed in at least one group. 
#From the manual: "The LRT examines two models for the counts, a full model with
# a certain number of terms and a reduced model, in which some of the terms of the full model are removed.
#The test determines if the increased likelihood of the data using the extra
# terms in the full model is more than expected if those extra terms are truly zero."
...
rds = here("data", "rnaseq","interim", "06_ddsLRT.Rds")
...
rds = here("data", "rnaseq","interim","06_ddsLRT_nolac.Rds")
...
rds = here("data", "rnaseq","interim", "06_ddsLRT_pam.Rds")
...
rds = here("data", "rnaseq","interim", "06_ddsLRT_pam_nolac.Rds")
...
rds = here("data", "rnaseq","interim", "06_ddsLRT_batch.Rds")
...
rds = here("data", "rnaseq","interim", "06_ddsLRT_batch_nolac.Rds")
