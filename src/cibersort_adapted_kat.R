...
#main function
CIBERSORT <- function(ref_profiles, target, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score',filename=filename,nCores=1){

  if(absolute && abs_method != 'no.sumto1' && abs_method != 'sig.score') stop("abs_method must be set to either 'sig.score' or 'no.sumto1'")

  #Testing
  #ref_profiles = sig_matrix

  #read in data
  X <- read.table(ref_profiles,header=T,sep="\t")
  rownames(X) <- X[,1]
  X <- X[,-1]
  Y <- read.table(target, header=T, sep="\t")
  ...
  Yorig <- Y
  #Ymedian <- max(median(Yorig),1) #Crashes when called on a matrix
  ...
  output <- foreach(itor = c(1:mixtures), .combine=rbind, .packages=c("e1071","parallel"), .export = "CoreAlg", .options.snow=opts) %dopar%{
  #while(itor <= mixtures){
    ...
    if(absolute && abs_method == 'sig.score') {
      ...
      print("Fix me") #Ymedian cannot be called on a matrix
    }
...
fileName <- "/DATA/share/postpartumbc/results/cibersort/cibersort_Rresults_brcasig_nperm100.csv"
sig_matrix <- "/DATA/share/postpartumbc/data/external/cibersort/20190527_refProfilesTCD_raw.csv"
target <- "/DATA/share/postpartumbc/data/RNA-seq/hugo_fpkm.txt"

results <- CIBERSORT(sig_matrix,target, perm = 100, QN = F, absolute=F , abs_method='sig.score',nCores=4,filename = fileName)
