...
DEAnalysisMAST<-function(scdata,id,path){
  ...
  for (i in unique(id)){
    ...
    if(dim(DE)[1]>1){
      ...
      head(data_for_MIST)
      print("finished get data_for_MIST")
      colnames(data_for_MIST) = c("Gene", "Subject.ID", "Et", "Population", "NumCells")
      vbeta = data_for_MIST
      vbeta.fa <-FromFlatDF(vbeta, idvars=c("Subject.ID"),
                            primerid='Gene', measurement='Et', ncells='NumCells',
                            geneid="Gene",  cellvars=c('NumCells', 'Population'),
                            phenovars=c('Population'), id='vbeta all')
      print("finished get FromFlatDF")
      vbeta.1 <- MAST::subset(vbeta.fa,NumCells==1)
      #vbeta.1 <- vbeta.fa[ which(vbeta.fa$NumCells==1), ]
      print("finished get subset")
      # .3 MAST
